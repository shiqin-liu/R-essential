...
```
Math in R, R can also do many other things using built-in method
```{r}
1+1
sqrt(2)
log(1000)

# more math operation; look to 
```
...
df1[2:3,c("Date","Strings")] # pull out the same using column names 

df2[c(1,3),"Date"]
df2$Date[c(1,3)] #these two do the same

# edit the dataframe
df2$Num <- as.character(df1$Num)
str(df1)
View(df1) #view certain datasets in a graphical manner 
...
list1
list1[[2]][2:4,3] # the objects in a list are indexed with the double bracket [[ ]]; in this case, it pick the second object in the list, and 2 to 4th row, 3rd column in that vector
